# Duckstring
A cloud-native ELT framework supporting orchestration of data preparation DAGs by breaking each stage into potentially-useful standalone nodes or "Ponds". The pipeline consists of deriving potentially multiple downstream Ponds from upstream Ponds, managing any version dependencies. The pipeline is built by dependency resolution from terminal ponds backwards up the stream.

The default engine is DuckDB with a MotherDuck backend (though this is configurable), and is focussed on incremental loads for tables of fewer than 50M rows, operating on a single compute node. 
...
- Brains and delays are more expensive than compute
- Communication is the core bottleneck

## Ponds
Each Pond is attached to a version-controlled repository. It should be treated like a code package, but contains each of:

- **Code** defining transformations (ideally incremental) from source tables (upstream Ponds) to target tables
- **Compute** defining the engine requirements and compute settings
- **Storage** defining the location of tables
- **Schedule** defining the conditions under which the Pond executes

### Public / Private
Tables in a Pond are either Public or Private - or, if you prefer, Over or Under water. 
...
Public tables are the 'result' of a Pond, and are all that any downstream Ponds can consume from.

All tables in a Pond must be uniquely named within each of Public and Private.
