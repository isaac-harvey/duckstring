...
| `DUCKSTRING_DUCK_SIZE` / `DUCKSTRING_DUCK_FLOCK` | `s` / `on` | Catchment-wide worker defaults (preset size; may work escalate to an offload engine). Per-Pond overrides via `duckstring duck set`. Inert on a stock local Catchment. |
