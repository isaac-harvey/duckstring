...
## Plan capture (experimental)

A [Trickle](../guides/trickle.md)-only Ripple can be **captured** instead of executed: `duckstring.trickle.capture.capture_plan(run_python, source_catalog=…)` runs the function against a recording handle whose `.trickle()` chains record rather than compute, and returns the pipeline as a portable **logical plan** (versioned JSON — the operator DAG, pipeline, aggregate/accumulate specs, and a catalog of the source tables). Capture is exact or loud: anything the plan can't express — a raw `pond.con` access, a callable metric (`agg.reduce`/`acc.scan`), an Ibis `.sql()` expression — raises `NonCapturable`, and the caller falls back to normal execution. Nothing about capture touches the executing path.

The first consumer is **execution offload** (`duckstring.duckflock_backend`, also experimental): with `DUCKFLOCK_BIN` pointing at a co-resident [DuckFlock](https://duckstring.com) driver CLI, the worker captures each eligible Ripple, asks the driver for a routing quote, and either runs it locally as normal or runs the plan through the driver (a subprocess) and folds the result back in — downstream Ripples, the publish, and the [version-contract gate](../concepts/versioning.md) behave identically either way. `DUCKSTRING_DUCK_FLOCK=off` keeps the driver but disables distribution (statements execute driver-local). Every failure on the offload side degrades to normal local execution with a warning; unset, nothing changes.
