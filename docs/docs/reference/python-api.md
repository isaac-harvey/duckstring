...
The first consumer is **execution offload** (`duckstring.duckflock_backend`, also experimental): with `DUCKSTRING_DUCKFLOCK_ENDPOINT` set, the worker captures each eligible Ripple, asks an external engine for a routing quote, and either runs it locally as normal or submits the plan and folds the result back in — downstream Ripples, the publish, and the [version-contract gate](../concepts/versioning.md) behave identically either way. Every failure on the offload side degrades to normal local execution with a warning; unset, nothing changes.
