...
A Pond's output schema is whatever its Ripples produce — so it can't be vetted before the Pond runs. Instead Duckstring checks it **at publish time**, before the new output overwrites the live tables. The rule for a major line is **additive-only**: a version that advances the line must keep every table and column the line already published (new tables and columns are fine, and so is a **lossless widening** of a column's type — `DECIMAL(24,2)` → `DECIMAL(25,2)`, `INTEGER` → `BIGINT` — since every existing value still fits; a dropped column, a removed table, or a **narrowing** type change is a breaking change).
...
- Widening matters more than it sounds. SQL result types follow the *expression*, so a Ripple that takes a data-dependent branch — a `COALESCE` over a lookup, say — can legitimately emit `DECIMAL(24,2)` on one run and `DECIMAL(25,2)` on the next with identical semantics. Treating that as breaking would fail the Pond, and because the gate is forward-only and a failed run publishes nothing, the line could never recover on its own. Still, the sturdy habit is to **`CAST` output columns to a declared type** so your published schema is decided by you rather than by today's data.
- If a line does get stuck behind a genuine narrowing, `duckstring control reset-contract {pond}` drops the recorded schema so the next accepted run re-freezes it. It re-opens what a pinned Sink was promised, so it's full-access and asks for confirmation.
